...
function openScriptureMenu() {
    ...
    // Update this path to match exactly where you put the audio file!
    scriptureAudio.src = 'audio/c2/scripture/Genesis-1.m4a'; 
...
function buildScriptureLyrics() {
    ...
    scriptureSongC2.forEach((line, index) => {
...
scriptureAudio.addEventListener('timeupdate', () => {
    ...
    // 1. Auto-Pause Logic
    const nextLine = scriptureSongC2.find(line => line.time > currentTime);
    ...
    let newActiveLine = -1;
    for (let i = scriptureSongC2.length - 1; i >= 0; i--) {
        if (currentTime >= scriptureSongC2[i].time) {
            newActiveLine = i;
            break;
        }
    }
